...
| `@changfenhuang/dsh-genui` (the one adopted plugin — Story 8.1) | MIT | 29 Aug 2026 — read the `LICENSE` file in the installed package at the pinned version `0.9.3`: the MIT licence text verbatim, Copyright (c) 2026 dsh-external. It resolves one runtime dependency of its own, `react` (MIT), and vendors three prebuilt engines no metadata field names — ECharts 5.6.0 (Apache-2.0), three.js 0.180.0 (MIT) and Mermaid 11.16.0 (MIT, carrying DOMPurify 3.4.13 dual `MPL-2.0 OR Apache-2.0`, Apache-2.0 elected). All four are declared in `docs/licence-decisions.json` and none of them is executed: the permitted component set is tables, charts and plots. |

 ...
 * title/favicon swap, both reached through `ctx.webServer`'s own extension
 * points (`register`, `tapIndex`) rather than any harness file edit.
 *
...
/** A stub `webServer` service that records what got registered, mirroring the real one's shape closely enough for these tests. */
function stubWebServerCtx() {
	const routes = [];
	const taps = [];
	return {
		routes,
...
test("registers an exact favicon route serving our svg, not the harness's", () => {
	const { ctx, routes } = stubWebServerCtx();
	apply(ctx);
...
test("taps index.html to swap the shipped title and favicon link for ours", () => {
	const { ctx, taps } = stubWebServerCtx();
	apply(ctx);
